 ...
 *Имя класса должно совпадать с ID модуля, вместо точки между именем парнера и именем модуля знак подчеркивания
 *Класс модуля должен быть наследником системного класса модулей Битрикс CModule*/
Class flight_control extends CModule
{
    /*Обязательные свойства объекта модуля*/
    const MODULE_ID = "flight.control";
    var $MODULE_ID = "flight.control";
    var $MODULE_VERSION;
    ...
    function __construct()
    {
        ...
        $this->MODULE_NAME = GetMessage("flight.control_MODULE_NAME");
        $this->MODULE_DESCRIPTION = GetMessage("flight.control_MODULE_DESC");

        $this->PARTNER_NAME = GetMessage("flight.control_PARTNER_NAME");
        $this->PARTNER_URI = GetMessage("flight.control_PARTNER_URI");
    }
    ...
    /*Метод установки (копирования файлов в ядро Битрикса) файлов модуля*/
    ...
    /*Метод удаления (удаления файлов из ядра Битрикса) файлов модуля*/
    ...
    /*Метод запускается при нажатии кнопки Установить на странице Модули административного раздела, осуществляет инсталляцию модуля*/
    /*ОБЯЗАТЕЛЬНЫЙ ДЛЯ РЕАЛИЗАЦИИ МЕТОД*/
    ...
    /*Метод запуска файлов SQL, которые содержат запросы на создание таблиц в БД для работы модуля*/
    function InstallDB()
    {
        global $DB, $DBType;
